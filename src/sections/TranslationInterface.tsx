...
import { 
  ...
  Shield, Zap, Globe, Maximize2, Eye, Sparkles, Minimize2
} from 'lucide-react';
...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({ label, value, onChange, disabled }) => {
  ...
  return (
    <div className="relative group w-full" ref={containerRef}>
      <label className="block text-xs text-foreground/40 uppercase tracking-wider mb-2 font-medium">
        {label}
      </label>
        ...
        >
          <div className="flex items-center gap-3" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
            ...
            <div className={direction === 'rtl' ? 'text-right' : 'text-left'}>
              <span className="text-foreground font-medium block">{value.name}</span>
              <span className="text-foreground/40 text-xs">{value.nameNative}</span>
            </div>
          </div>
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 max-h-60 overflow-y-auto glass-futuristic rounded-xl shadow-2xl z-50 animate-fade-in-up border border-white/10">
            <div className="sticky top-0 glass-futuristic p-3 border-b border-white/10 z-10">
              <p className="text-[10px] text-foreground/40 uppercase tracking-widest">{t('translate.selectLang')}</p>
            </div>
            <div className="p-1">
              {LANGUAGES.map((lang) => (
                <button
                ...
                >
                  <div className="flex items-center gap-3" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
                    ...
                    <div>
                      ...
                      <span className="text-xs text-foreground/40">{lang.nameNative}</span>
                    </div>
                  </div>
...
const InlineDocumentViewer: React.FC<InlineDocumentViewerProps> = ({ 
  ...
  // All lines for continuous scroll
  const allLines = translatedContent.split('\n');

  if (isExpanded) {
    // Fullscreen view - Continuous vertical scroll
    return (
      <div className="fixed inset-0 z-[200] animate-fade-in bg-slate-950/95 backdrop-blur-2xl">
        {/* Toolbar */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 glass-futuristic border-b border-white/10">
          <div className="flex items-center gap-3" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
            ...
            <div className={direction === 'rtl' ? 'text-right' : 'text-left'}>
              ...
              <div className="flex items-center gap-2 text-xs text-foreground/50" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2.5 text-foreground/50 hover:text-foreground hover:bg-white/10 rounded-lg transition-all"
            >
        ...
        {/* Document Content - Continuous vertical scroll */}
        <div 
          ref={contentRef}
        ...
        >
          <div className="p-6 sm:p-10 lg:p-16">
            ...
            >
              ...
              {/* Content - All lines in continuous flow */}
              ...
              <div className="border-t border-gray-200 pt-4 mt-8">
                <div className="flex items-center justify-center">
                  <span className="text-xs text-gray-400">— {direction === 'rtl' ? 'نهاية المستند' : 'End of Document'} —</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <section id="translation-section" className="relative py-20">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-futuristic border border-teal-500/30 mb-6">
            <Sparkles className="w-4 h-4 text-teal-400" />
            <span className="text-sm text-teal-400 font-medium">{t('translate.badge')}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {[
            { icon: Shield, title: t('translate.feature1.title'), desc: t('translate.feature1.desc'), color: 'teal' },
            ...
            <div 
              key={index} 
              className="glass-futuristic p-4 flex items-center gap-3 hover:border-teal-500/30 transition-all duration-300 hover:scale-[1.02] animate-fade-in-up rounded-2xl"
            ...
            >
              ...
              <div>
                ...
                <p className="text-foreground/50 text-xs">{feature.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="relative">
          {/* Glow effect - Teal */}
          <div className="absolute -inset-1 bg-gradient-to-r from-teal-500/20 via-teal-600/20 to-teal-500/20 rounded-3xl blur-xl opacity-50 animate-pulse" />
          
          <div className="relative glass-futuristic rounded-3xl p-1 overflow-hidden">
            <div className="bg-slate-950/40 rounded-[22px] relative min-h-[500px] flex flex-col">
              {/* Ambient glows - Teal */}
              <div className="absolute inset-0 overflow-hidden rounded-[22px] pointer-events-none">
                <div className="absolute top-0 right-0 w-96 h-96 bg-teal-500/10 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 animate-pulse" />
                <div className="absolute bottom-0 left-0 w-96 h-96 bg-teal-600/10 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2 animate-pulse" style={{ animationDelay: '1s' }} />
              </div>
              ...
              <div className="p-8 md:p-12 relative z-10 flex flex-col flex-1">
                ...
                {status !== TranslationStatus.COMPLETED && (
                  <div className="flex flex-col md:flex-row gap-4 justify-between items-center mb-8 animate-fade-in-up">
                    ...
                    >
                      <ArrowRightLeft className="h-5 w-5 text-teal-400 group-hover:text-teal-300 transition-colors" />
                    </button>
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  {status !== TranslationStatus.COMPLETED && status !== TranslationStatus.TRANSLATING && status !== TranslationStatus.WATCHING_AD && (
                    <div 
                      className={`border-2 border-dashed rounded-2xl transition-all duration-500 flex flex-col items-center justify-center p-10 text-center cursor-pointer flex-1 min-h-[250px] hover:scale-[1.02] ${
                        file 
                          ...
                          : 'border-white/20 hover:border-teal-500/30 hover:bg-white/5'
                      }`}
                    ...
                    >
                      ...
                      <div className={`relative mb-5 transition-all duration-500 ${file ? 'scale-110' : ''}`}>
                        ...
                        <div className="relative h-20 w-20 rounded-2xl bg-gradient-to-br from-teal-500/20 to-teal-600/20 flex items-center justify-center border border-teal-500/30">
                          {file ? <FileCheck className="text-teal-400 h-10 w-10" /> : <Upload className="text-teal-400 h-10 w-10" />}
                        </div>
                      </div>
                      ...
                      <p className="text-foreground/50 max-w-md mx-auto mb-3 text-sm">
                        {file 
                      ...
                      {!file && (
                        <div className="flex items-center gap-2 text-sm text-teal-400 hover:text-teal-300 transition-colors">
                  ...
                  {status === TranslationStatus.TRANSLATING && (
                    <div className="flex flex-col items-center justify-center flex-1 animate-fade-in">
                      <div className="relative mb-6">
                        ...
                        <div className="relative h-24 w-24 rounded-full bg-gradient-to-br from-teal-500/20 to-teal-600/20 flex items-center justify-center border border-teal-500/30">
                  ...
                  {status === TranslationStatus.CANCELLED && (
                    <div className="flex flex-col items-center justify-center flex-1 animate-fade-in">
                      ...
                      <h3 className="text-xl font-bold text-foreground mb-2">{t('translate.cancelled.title')}</h3>
                      <p className="text-foreground/50 mb-4">{t('translate.cancelled.desc')}</p>
                      <button 
                        onClick={() => setStatus(TranslationStatus.FILE_SELECTED)}
                        className="px-6 py-3 glass-futuristic rounded-xl text-foreground hover:bg-white/10 transition-all hover:scale-105"
                      >
                  ...
                  {status === TranslationStatus.ERROR && (
                    <div className="text-center py-8 flex-1 flex flex-col items-center justify-center animate-fade-in">
                      ...
                      <h3 className="text-xl font-bold text-foreground mb-2">{t('translate.error.title')}</h3>
                      <p className="text-foreground/50 mb-6 max-w-md text-sm">{error}</p>
                      <button 
                        onClick={reset} 
                        className="px-6 py-3 glass-futuristic rounded-xl text-foreground hover:bg-white/10 transition-all border border-white/20 hover:scale-105"
                      >
                ...
                {status === TranslationStatus.FILE_SELECTED && (
                  <div className="mt-6 flex justify-center animate-fade-in-up">
                    <button 
                      ...
                      className="group relative px-8 py-4 bg-gradient-to-r from-teal-500 via-teal-600 to-teal-500 text-slate-900 font-bold text-lg rounded-2xl overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-xl shadow-teal-500/25"
                    >
                      <span className="relative z-10 flex items-center gap-3">
                        <Sparkles className="h-5 w-5" />
                        {direction === 'rtl' ? 'شاهد إعلان وترجم' : 'Watch Ad & Translate'}
                      </span>
                      <div className="absolute inset-0 bg-gradient-to-r from-teal-400 via-teal-500 to-teal-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                ...
                {status === TranslationStatus.COMPLETED && (
                  <div className="mt-6 flex justify-center animate-fade-in-up">
                    <button 
                      onClick={reset}
                      className="px-6 py-3 glass-futuristic text-foreground rounded-xl hover:bg-white/10 transition-all font-medium border border-white/20 hover:scale-105"
                    >
                      <RefreshCcw className="h-4 w-4 inline mr-2" />
                      {t('translate.newFile')}
                    </button>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <p className="w-full text-center text-foreground/40 text-xs mb-2">{t('translate.supported')}</p>
          {['PDF', 'TXT', 'MD', 'DOC', 'CSV'].map((format) => (
            <div 
              key={format}
              className="px-3 py-1.5 glass-futuristic rounded-lg text-xs text-foreground/60 flex items-center gap-2 hover:border-teal-500/30 transition-all"
            >
        ...
        {status === TranslationStatus.COMPLETED && file && (
          <div ref={resultRef} className="mt-12 animate-fade-in-up">
            ...
            <div className="relative">
              ...
              <div className="relative glass-futuristic rounded-3xl p-1 overflow-hidden">
                <div className="bg-slate-950/40 rounded-[22px] p-6 sm:p-8">
                  {/* Result Header */}
                  <div className="flex items-center justify-between mb-6" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
                    <div className="flex items-center gap-4" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
                      ...
                      <div className={direction === 'rtl' ? 'text-right' : 'text-left'}>
                        ...
                        <div className="flex items-center gap-2 text-sm text-foreground/50" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
                  ...
                  {!showInlineDocument ? (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <div className="bg-slate-950/80 backdrop-blur-sm px-4 py-2 rounded-full flex items-center gap-2">
                    ...
                    >
                      ...
                      <button
                        ...
                        className="mt-4 w-full py-3 glass-futuristic text-foreground/70 hover:text-foreground rounded-xl transition-all text-sm"
                      >
                  ...
                  {!showInlineDocument && (
                    <div className="mt-6">
                      <button
                        onClick={handleViewDocument}
                        className="w-full py-3.5 bg-gradient-to-r from-teal-500 to-teal-600 text-slate-900 font-bold rounded-xl hover:from-teal-400 hover:to-teal-500 transition-all hover:scale-[1.02] shadow-lg shadow-teal-500/25 flex items-center justify-center gap-2"
                      >
