...
export function AdOverlay({ isOpen, onClose, onComplete, fileName, sourceLang, targetLang }: AdOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100]">
      {/* Warning Modal */}
      {showWarning && (
        <div className="absolute inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-fade-in">
          <div className="relative w-full max-w-md futuristic-card rounded-3xl p-8 animate-scale-in corner-accent">
            ...
            <div className="relative flex flex-col items-center text-center">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-yellow-500/20 to-orange-500/20 flex items-center justify-center mb-6 neon-glow-teal">
              ...
              <div className="flex gap-3 w-full">
                <button
                  ...
                  className="flex-1 py-3.5 glass-futuristic text-foreground rounded-xl hover:bg-white/10 transition-all font-medium"
                >
                  {direction === 'rtl' ? 'إلغاء' : 'Cancel'}
                </button>
                <button
                  onClick={handleConfirmClose}
                  className="flex-1 py-3.5 bg-gradient-to-r from-teal-500 to-teal-600 text-slate-900 rounded-xl hover:from-teal-400 hover:to-teal-500 transition-all font-medium shadow-lg shadow-teal-500/25"
                >
      ...
      {/* Main Overlay */}
      <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-2xl flex flex-col animate-fade-in">
        {/* === PROGRESS BANNER AT TOP === */}
        <div className="relative z-20 bg-gradient-to-b from-slate-950/80 to-transparent">
          {/* Progress bar */}
          <div className="h-1.5 bg-white/10">
            ...
            >
              <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-teal-400 rounded-full blur-md" />
          ...
          <div className="flex items-center justify-between px-4 sm:px-8 py-4">
            <div className="flex items-center gap-4" style={{ flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }}>
              <div className="relative">
                ...
                <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-teal-500/20 to-teal-600/20 flex items-center justify-center border border-teal-500/30">
                  <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-teal-400" />
                </div>
              </div>
              <div className={direction === 'rtl' ? 'text-right' : 'text-left'}>
                <p className="text-foreground font-medium text-sm sm:text-base truncate max-w-[150px] sm:max-w-[250px]">{fileName}</p>
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end">
                ...
                <span className={`text-2xl font-bold ${isComplete ? 'text-green-400' : 'text-teal-400'}`}>
                  {Math.round(progress)}%
                </span>
              </div>
              <button 
                onClick={handleCloseClick}
                className="p-2.5 text-foreground/50 hover:text-foreground hover:bg-white/10 rounded-xl transition-all duration-300"
              >
                  ...
                  <div key={index} className="flex items-center gap-2">
                    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all duration-300 ${
                      ...
                      <div className={`w-2 h-2 rounded-full ${
                        isActive ? 'bg-green-400' : isCurrent ? 'bg-teal-400 animate-pulse' : 'bg-foreground/30'
                      }`} />
        ...
        <div className="flex-1 flex items-center justify-center p-6 sm:p-12 relative">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-teal-500/5 rounded-full blur-[150px] animate-pulse" />
            <div className="absolute bottom-1/3 left-1/2 -translate-x-1/2 w-[400px] h-[400px] bg-teal-600/5 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '1s' }} />
          </div>
          ...
          {!isComplete ? (
            ...
            <div className="relative w-full max-w-5xl flex flex-col items-center justify-center">
              ...
              <div className="relative w-full aspect-video max-h-[60vh] min-h-[400px] futuristic-card rounded-3xl overflow-hidden corner-accent shadow-2xl shadow-teal-500/10">
                {/* Ad placeholder - in real app, this would be the actual ad */}
                <div className="absolute inset-0 bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 flex flex-col items-center justify-center">
                  {/* Animated ad content */}
                  <div className="relative">
                    <div className="absolute inset-0 bg-teal-500/20 rounded-full blur-2xl animate-pulse" />
                    <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-teal-500/30 border-t-teal-500 animate-spin" />
                  </div>
                  
                  <div className="mt-8 text-center">
                    <p className="text-foreground/60 text-lg sm:text-xl mb-2">
            ...
            <div className="flex flex-col items-center animate-scale-in">
              ...
              <h3 className="text-3xl sm:text-4xl font-bold text-foreground mb-3 text-center">
                {direction === 'rtl' ? 'اكتملت الترجمة!' : 'Translation Complete!'}
              </h3>
              <p className="text-foreground/50 text-sm sm:text-base mb-8 text-center max-w-md">
                {direction === 'rtl' 
              ...
              <button
                ...
                className="group relative px-10 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-slate-900 font-bold text-lg rounded-2xl overflow-hidden transition-all hover:scale-105 shadow-xl shadow-green-500/25"
              >
        ...
        {/* Bottom status bar */}
        <div className="relative z-20 px-4 sm:px-8 py-4 border-t border-white/5 bg-slate-950/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${isComplete ? 'bg-green-400' : 'bg-teal-400 animate-pulse'}`} />
            ...
            <div className="text-xs text-foreground/30 flex items-center gap-2">
